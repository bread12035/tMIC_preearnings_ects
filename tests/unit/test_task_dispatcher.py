"""Tests for event_calendar.task_dispatcher_main.run_dispatch (sync)."""
...
def test_publishes_due_tasks_and_marks_published(fake_gcs) -> None:
    ...
    publisher = MagicMock()
    publisher.publish = MagicMock(return_value="msg-1")
...
def test_skips_tasks_outside_window(fake_gcs) -> None:
    """exec_time is now+11min, dispatch_window=10min: must NOT publish."""
    now = datetime(2026, 4, 28, 20, 0)
    reg = TaskRegistry(fake_gcs, BUCKET, PREFIX)
    reg.save("2026", "Q2", [_task(exec_time=now + timedelta(minutes=11))])
    ...
    assert reg.load("2026", "Q2")[0].status == "pending"


def test_window_boundary_inclusive(fake_gcs) -> None:
    """exec_time == now + window: at the boundary, the task IS dispatched."""
    now = datetime(2026, 4, 28, 20, 0)
    reg = TaskRegistry(fake_gcs, BUCKET, PREFIX)
    reg.save("2026", "Q2", [_task(exec_time=now + timedelta(minutes=10))])
...
def test_already_published_tasks_skipped(fake_gcs) -> None:
    now = datetime(2026, 4, 28, 20, 25)
    reg = TaskRegistry(fake_gcs, BUCKET, PREFIX)
    reg.save("2026", "Q2", [_task(status="published")])
...
def test_publish_failure_keeps_task_pending(fake_gcs) -> None:
    ...
    publisher = MagicMock()
    publisher.publish = MagicMock(side_effect=RuntimeError("pubsub down"))
...
def test_multiple_quarters_handled(fake_gcs) -> None:
    now = datetime(2026, 4, 28, 20, 25)
    reg = TaskRegistry(fake_gcs, BUCKET, PREFIX)
