...
def _make_client(max_retries: int = 3) -> ClaudeClient:
    # Avoid real network init by using a placeholder API key (Anthropic
...
def test_complete_returns_text_on_success() -> None:
    ...
    client._client.messages.create = MagicMock(return_value=_ok_response("hello"))

    out = client.complete(system="sys", user_prompt="user")
    assert out == "hello"
...
def test_4xx_does_not_retry() -> None:
    ...
    with pytest.raises(ClaudeAPIError):
        client.complete(system="s", user_prompt="u")
    assert client._client.messages.create.call_count == 1


def test_rate_limit_exhausts_after_max_retries() -> None:
    client = _make_client(max_retries=3)
    client._client = MagicMock()
    ...
    with patch("common.claude_client.time.sleep"):
...
def test_5xx_retried_then_succeeds() -> None:
    ...
    with patch("common.claude_client.time.sleep"):
        out = client.complete(system="s", user_prompt="u")
