...
def test_read_bytes_maps_notfound_to_gcsnotfound() -> None:
    ...
    with pytest.raises(GCSObjectNotFound):
        svc.read_bytes("bk", "missing")


def test_write_text_uploads() -> None:
    svc, mock_client = _make_service_with_mocked_client()
    mock_blob = MagicMock()
    mock_client.bucket.return_value.blob.return_value = mock_blob

    svc.write_text("bk", "path", "hi")
    mock_blob.upload_from_string.assert_called_once()
    args, kwargs = mock_blob.upload_from_string.call_args
    assert args[0] == "hi"


def test_write_text_wraps_errors() -> None:
    svc, mock_client = _make_service_with_mocked_client()
    mock_blob = MagicMock()
    ...
    with pytest.raises(GCSWriteError):
        svc.write_text("bk", "path", "x")


def test_read_json_round_trip() -> None:
    svc, mock_client = _make_service_with_mocked_client()
    mock_blob = MagicMock()
    mock_blob.download_as_bytes.return_value = b'{"k": 1}'
    mock_client.bucket.return_value.blob.return_value = mock_blob

    out = svc.read_json("bk", "p")
    assert out == {"k": 1}


def test_exists_calls_blob_exists() -> None:
    svc, mock_client = _make_service_with_mocked_client()
    mock_blob = MagicMock()
    mock_blob.exists.return_value = True
    mock_client.bucket.return_value.blob.return_value = mock_blob

    assert svc.exists("bk", "p") is True
